...
// ─── Cursor-optimized full kit text ──────────────────────────────────

export function kitToCursorText(kit: PromptKit): string {
  const f = kit.foundation;

  let md = `# ${kit.projectName} — Prompt Kit\n`;
  md += `# Generated by Provibal (provibal.com)\n`;
  md += `# Type: ${kit.projectType} | Stack: ${kit.techStack}\n\n`;
  md += `---\n\n`;
  md += `## FOUNDATION\n\n`;
  md += `### Identity\n${f.identity}\n\n`;
  ...
  md += `---\n\n`;
  md += `## PROJECT MAP\n\n`;
  md += `${kit.projectMap.overview}\n\n`;

  kit.projectMap.fileStructure.forEach((file) => {
    md += `${file.filePath} — ${file.responsibility}\n`;
  });

  md += `\n---\n\n`;
  md += `## BUILD SEQUENCE\n\n`;
  kit.featureSequence.steps.forEach((step) => {
    md += `### Step ${step.order}: ${step.featureName}\n\n`;
    md += `${step.prompt}\n\n`;
    if (step.filesToCreate.length > 0) {
      md += `Files: ${step.filesToCreate.join(", ")}\n\n`;
    }
  });

  md += `---\n\n`;
  md += `## PROVIBAL PROTOCOL\n\n`;

  if (kit.followUpChain && kit.followUpChain.prompts.length > 0) {
    kit.followUpChain.prompts.forEach((p) => {
      md += `### ${p.order}. ${p.title}\n`;
      md += `Purpose: ${p.purpose}\n\n`;
      md += `${p.prompt}\n\n`;
    });
  } else {
    md += `Not generated yet.\n`;
  }
